...
function IndexPopup() {
  ...
  const validateTabUrl = (): void => {
    ...
    setIsMoviePage(
      activeTab?.url?.includes('kinopoisk.ru/film/') || activeTab.url.includes('kinopoisk.ru/series/')
    )
  }
  ...
  return (
    <Container $isInvalid={!isMoviePage}>
      ...
      <ServicesList>
        {services.map((item) => (
          <ServiceItem activeTab={activeTab} item={item} key={item.name} isMoviePage={isMoviePage} />
        ))}
      </ServicesList>
    </Container>
  )
